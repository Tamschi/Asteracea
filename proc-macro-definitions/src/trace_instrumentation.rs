		...
		ItemFn {
			block: {
				...
				let inner = quote_spanned! {self.function.sig.output.span().resolved_at(Span::mixed_site())=>
					let result: ::std::result::Result<_, ::#asteracea::error::Escalation> = (|| { #statements })();
					result.map_err(|gui_error| gui_error.__Asteracea__with_traced_frame(::std::borrow::Cow::Borrowed(::std::concat!(
